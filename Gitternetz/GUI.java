...
public class GUI extends JFrame {
    ...
    public GUI(Gitter g) {
        ...
        d = new JSlider(1, 600,
                        (int) (Gitter.getDaempfung() * 1000.0));
        t = new JSlider(1, 600,
                        (int) (Gitter.getTraegheit() * 1000.0));
        r = new JSlider(70, 600,
                        (int) (Gitter.getRuheabstand() * 1000.0));
        s = new JSlider(0, 5000,
                        (int) (Gitter.getSchwerkraft() * 100000.0));
                ...
                if (e.getButton() == MouseEvent.BUTTON2) {
                    ...
                    for (int y = 0; y < gitter.getHoehe(); y++) {
                        for (int x = 0; x < gitter.getBreite(); x++) {
                            ...
                            if (abstand(x1, y1, e.getX(), e.getY())
                                <= radius * 2.0) {
                                swing_x = x;
                                swing_y = y;
                                return;
        ...
        addMouseMotionListener(new MouseMotionListener() {
            ...
            public void mouseDragged(MouseEvent e) {
                ...
                dragged.setX(dragged.getX()
                             + dx * Gitter.getRuheabstand() / 7.0);
                dragged.setZ(dragged.getZ()
                             + dy * Gitter.getRuheabstand() / 7.0);
    ...
    public void paint(Graphics g) {
        ...
        for (int y = 0; y < gitter.getHoehe(); y++) {
            for (int x = 0; x < gitter.getBreite(); x++) {
                ...
                if (show) {
                    bufg.setColor(Color.black);
                    bigPoint(bufg, (int)(x1 + y1 / 2.0),
                             getHeight() - (int)(z1 + y1 / 2.0), 2);
                }
                ...
                for (Knoten i: k.getNachbarn()) {
                    ...
                    bufg.setColor(c);
                    bufg.drawLine((int)(x1 + y1 / 2.0),
                                  getHeight() - (int)(z1 + y1 / 2.0),
                                  (int)(x2 + y2 / 2.0),
                                  getHeight() - (int)(z2 + y2 / 2.0));
                }
            }
        }
        ...
        if (dragged != null) {
            ...
            if (!swing) {
                ...
                bigPoint(bufg, (int)(x1 + y1 / 2.0),
                         getHeight() - (int)(z1 + y1 / 2.0), radius);
            }
        }
